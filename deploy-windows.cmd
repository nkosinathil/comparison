...
if errorlevel 1 (
  echo ERROR: SSH pre-check failed for App Server. Configure SSH key-based login for %SSH_USER_APP%@%APP_HOST% ^(e.g., run ./setup-ssh-keys.sh %SSH_USER_APP%@%APP_HOST% in Git Bash or WSL^).
  exit /b 1
)
ssh %SSH_OPTS% %SSH_USER_PY%@%PYTHON_HOST% "echo ok"
if errorlevel 1 (
  echo ERROR: SSH pre-check failed for Python Server. Configure SSH key-based login for %SSH_USER_PY%@%PYTHON_HOST% ^(e.g., run ./setup-ssh-keys.sh %SSH_USER_PY%@%PYTHON_HOST% in Git Bash or WSL^).
  exit /b 1
)
ssh %SSH_OPTS% %SSH_USER_SSO%@%SSO_HOST% "echo ok"
if errorlevel 1 (
  echo ERROR: SSH pre-check failed for SSO Server. Configure SSH key-based login for %SSH_USER_SSO%@%SSO_HOST% ^(e.g., run ./setup-ssh-keys.sh %SSH_USER_SSO%@%SSO_HOST% in Git Bash or WSL^).
  exit /b 1
)
